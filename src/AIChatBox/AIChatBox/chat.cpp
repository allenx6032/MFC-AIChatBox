...
void Chat::printActorChaTag(std::string_view actor_name, CEdit& edit)
{
	actor_t actor = actors[actor_name.data()];
...
void Chat::printActorChaTag(std::string_view actor_name, CRichEditCtrl& edit, COLORREF color)
{
    actor_t actor = actors[actor_name.data()];
    std::string tag = actor.icon + " " + actor.name + ":";
    CStringW utf16String = ConvertUtf8ToUtf16(tag);
